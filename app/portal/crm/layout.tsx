...
export default async function CRMGuardLayout({
  children,
}: {
  ...
  if (token) {
    try {
      ...
      if (user.permissions?.includes('crm') || user.permissions?.includes('users')) {
        hasAccess = true;
      }
    } catch(e) {}
  }
      ...
      <div className="min-h-screen bg-slate-50 pt-20 flex items-center justify-center">
        <div className="bg-white rounded-3xl shadow-lg border border-slate-200 p-12 text-center max-w-lg">
          ...
          <p className="text-slate-500">
            Bạn không có quyền truy cập tính năng Quản lý Khách hàng (CRM). Hãy liên hệ Quản trị viên để được cấp dải quyền "crm".
          </p>
        </div>
      </div>
    );
